...
from game_agent import (myCustomPlayer, custom_score,custom_score_2,custom_score_3)
...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    print("\n{:^9}{:^13}{:^13}{:^13}{:^13}{:^13}".format(
        "Match #", "Opponent", test_agents[0].name,test_agents[1].name,test_agents[2].name, test_agents[3].name))
...
def main():
    ...
    ALPHA_BETA_ARGS = {"search_depth": 5, "search_type": 'alphabeta', "iterative_deepening": False}
    MINIMAX_ARGS = {"search_depth": 3, "search_type": 'minimax', "iterative_deepening": False}
    DEV_ARGS = {"search_depth": 5, "search_type": 'alphabeta', 'iterative_deepening': True}
    
    test_agents = [
        Agent(myCustomPlayer(score_fn=improved_score, **DEV_ARGS), "ID_Improved"),
        Agent(myCustomPlayer(score_fn=custom_score, **DEV_ARGS), "Student_1"),
        Agent(myCustomPlayer(score_fn=custom_score_2, **DEV_ARGS), "Student_2"),
        Agent(myCustomPlayer(score_fn=custom_score_3, **DEV_ARGS), "Student_3")
    ]

    # Define a collection of agents to compete against the test agents
    cpu_agents = [
        Agent(RandomPlayer(), "Random"),
        Agent(myCustomPlayer(score_fn=open_move_score , **MINIMAX_ARGS), "MM_Open"),
        Agent(myCustomPlayer(score_fn=center_score , **MINIMAX_ARGS), "MM_Center"),
        Agent(myCustomPlayer(score_fn=improved_score , **MINIMAX_ARGS), "MM_Improved"),
        Agent(myCustomPlayer(score_fn=open_move_score , **ALPHA_BETA_ARGS), "AB_Open"),
        Agent(myCustomPlayer(score_fn=center_score, **ALPHA_BETA_ARGS), "AB_Center"),
        Agent(myCustomPlayer(score_fn=improved_score, **ALPHA_BETA_ARGS), "AB_Improved")
    ]
