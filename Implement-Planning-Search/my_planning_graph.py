...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Implemenation as per AIMA 3rd Edition Chapter 10.3 (Planning Graphs)
        action_nodes = []
        
        # An action node at time i contains all the actions that could have their preconditions satisfied at that time i
        # The following loop determines what actions to add and create a node
        for action in self.all_actions:
            action_node = PgNode_a(action)
            #  Check if all the preconditions are satisfied by the previous literals for this action to hold
    ...
    def add_literal_level(self, level):
        ...
        # A literal S at time i contains all the literals that could hold at that time i, depending on the actions executed
        # at the preceding time steps.
        for action_level in self.a_levels[level -1]:
            
            # A literal appears because of an effect caused by the action
            # Add a literal (Node S) if there was an effect produced by an action
            for effect_nodes in action_level.effnodes:
                # Add the effect node to level state node
                literal_nodes.add(effect_nodes)
                effect_nodes.parents.add(action_level)
                action_level.children.add(effect_nodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_negates_a2 = any([effect in node_a2.action.effect_rem for effect in node_a1.action.effect_add])
        a2_negates_a1 = any([effect in node_a1.action.effect_rem for effect in node_a2.action.effect_add])
        
        
        return (a1_negates_a2 or a2_negates_a1)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_add_interferes_a2 = any([effect in node_a2.action.precond_neg for effect in  node_a1.action.effect_add])
        a1_rem_interferes_a2 = any([effect in node_a2.action.precond_pos for effect in  node_a1.action.effect_rem])
        a2_add_interferes_a1 = any([effect in node_a1.action.precond_neg for effect in  node_a2.action.effect_add])
        a2_rem_interferes_a1 = any([effect in node_a1.action.precond_pos for effect in  node_a2.action.effect_rem])
        
        return (a1_add_interferes_a2 or a1_rem_interferes_a2 or a2_add_interferes_a1 or a2_rem_interferes_a1)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        mutex = any([p1.is_mutex(p2) for p1 in node_a1.parents for p2 in node_a2.parents])        
        return mutex
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        mutex = node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos   
        return mutex
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        inconsistent_support = all([p1.is_mutex(p2) for p1 in node_s1.parents for p2 in node_s2.parents])  
    ...
    def h_levelsum(self) -> int:
        ...
        # Search for a goal in literal nodes at all the levels        
        for goal in self.problem.goal:
            for i, levels in enumerate (self.s_levels):
                if PgNode_s(goal, True) in levels:
                        level_sum += i
                        break
